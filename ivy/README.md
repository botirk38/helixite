...
Ivy runs **in-process** with no server, daemon, CLI, or HTTP API. It is not wire/API compatible with HelixDB.
...
let mut db = IvyBuilder::new().open(path)?;
...
Ivy is a self-contained Rust library. It does not include:
