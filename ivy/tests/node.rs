use ivy::{IvyBuilder, IvyError, Value};
...
fn test_mutate_node_remove_property() {
    ...
    db.batch(|tx| tx.node(id).remove_property("age").apply())
        .unwrap();
...
fn test_mutate_nonexistent_node_errors() {
    ...
    let result = db.batch(|tx| tx.node(999).apply());
    assert!(matches!(result, Err(IvyError::NodeNotFound(999))));
}
...
fn test_mutate_node_empty_apply_is_noop() {
    ...
    db.batch(|tx| tx.node(id).apply()).unwrap();
...
fn test_delete_node() {
    ...
    assert!(matches!(result, Err(IvyError::NodeNotFound(_))));
}
...
fn test_delete_node_persists_after_reopen() {
    ...
    assert!(matches!(result, Err(IvyError::NodeNotFound(1))));
