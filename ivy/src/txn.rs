...
impl<'a> ReadTxn<'a> {
    ...
    pub fn get_edge(&self, id: EdgeId) -> Result<Edge> {
        let bytes = self
            ...
            .ok_or(IvyError::EdgeNotFound(id))?;
...
impl<'a> WriteTxn<'a> {
    ...
    pub fn add_node(
        ...
        let bytes = bincode::serialize(&node).map_err(|e| IvyError::Codec(e.to_string()))?;
    ...
    pub fn add_edge(
        ...
        if self.txn.get(Db::Nodes, &from.to_be_bytes())?.is_none() {
            return Err(IvyError::NodeNotFound(from));
        }
        if self.txn.get(Db::Nodes, &to.to_be_bytes())?.is_none() {
            return Err(IvyError::NodeNotFound(to));
        }
        ...
        let bytes = bincode::serialize(&edge).map_err(|e| IvyError::Codec(e.to_string()))?;
...
fn next_id(txn: &mut dyn StorageWriteTxn, key: &[u8], name: &str) -> Result<u64> {
    ...
    let bytes: [u8; 8] = bytes
        .try_into()
        .map_err(|_| IvyError::Storage(format!("invalid {name} metadata value")))?;

    let next_id = u64::from_be_bytes(bytes);

    let following_id = next_id
        .checked_add(1)
        .ok_or_else(|| IvyError::Storage(format!("{name} overflow")))?;
...
impl<S: StorageEngine> NodeMutBuilder<'_, S> {
    pub(crate) fn new(db: &Ivy<S>, id: NodeId) -> NodeMutBuilder<'_, S> {
        NodeMutBuilder {
            db,
            id,
            ops: Vec::new(),
...
impl<S: StorageEngine> EdgeMutBuilder<'_, S> {
    pub(crate) fn new(db: &Ivy<S>, id: EdgeId) -> EdgeMutBuilder<'_, S> {
        EdgeMutBuilder {
            db,
            id,
            ops: Vec::new(),
